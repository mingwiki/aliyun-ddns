...
const {
    AccessKeyID, AccessKeySecret, Domains, ipv4_servers, ipv6_servers, ip_enabled
} = process.env
const key = {
...
const parseEnv = (s) => s
    ...
    .map((i) => i.trim())
    .filter((i) => !!i)
const [DomainNames, ipv4s, ipv6s, enabled] = [parseEnv(Domains), parseEnv(ipv4_servers), parseEnv(ipv6_servers), parseEnv(ip_enabled)]
...
export {
    DomainNames, ipv4s, ipv6s, enabled, ipv4Reg, ipv6Reg, log, request, update, getIp,
}
